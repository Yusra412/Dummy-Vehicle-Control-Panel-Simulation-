...
class VehicleControlPanel:
    ...
    def create_position_displays(self, parent: tk.Widget) -> None:
        """Create position and angle displays."""
        positions = [
            ("POS X [m]", "pos_x_m"),
            ("POS Y [m]", "pos_y_m"),
            ("POS Z [m]", "pos_z_m")
        ...
        for i, ((pos_label, pos_attr), (ang_label, ang_attr)) in enumerate(zip(positions, angles)):
            row_frame = tk.Frame(parent, bg='#404040')
            row_frame.grid(row=i, column=0, sticky='ew', padx=5, pady=2)  # Using grid for better control
    ...
    def create_gear_selector(self, parent: tk.Widget) -> None:
        ...
        self.canvases['gear_selector'] = tk.Canvas(gear_selector_frame, width=50, height=120, bg='black', highlightthickness=1, highlightbackground='white')  # Reduced size
    ...
    def update_gui(self) -> None:
        ...
        try:
            ...
            for gear in ['P', 'R', 'N', 'D', 'L', '1', '2', '3', '4', '5', '6']:
